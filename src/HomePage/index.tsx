...
class HomePageComponent extends PureComponent<TComponentProps, TComponentState> {
  ...
  public render() {
    ...
    return (
      <Container fluid className={classnames(styles.container, { [styles.alertDismissed]: isCovidAlertDismissed })}>
            ...
            <FadeIn>
              {({ className, ref }: { className: string; ref: React.Ref<HTMLElement> }) => (
                <Typography ref={ref} variant="h4" align="center" className={classnames(styles.text, styles.heroText, className)}>
                  {t('build_your_professional_resume_in_10')}
                </Typography>
              )}
            </FadeIn>
          </Col>
            ...
            <Row className={className} ref={ref}>
              <Col>
                <Card className={styles.resumeCard}>
                  <img src={sampleResume} className={styles.resume} />
                </Card>
              </Col>
            </Row>
          )}
